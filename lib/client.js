...
`
    ...
    const DICT = {
      ...
      en: {
        ...
        urlImportGo: 'Fetch',
        badgeOn: '{n} permission rules active · click to open settings',
    ...
    function PermissionsBadge(props) {
      ...
      const count = info.enabled
        ...
        : -1
      const onClick = () => {
        // 尽力打开设置：派发键盘事件不可行，走 shell 命令面板的公开事件（若无则静默）
        try { window.dispatchEvent(new CustomEvent('dsh-open-settings', { detail: { section: 'dsh-permissions' } })) } catch (e) {}
      }
