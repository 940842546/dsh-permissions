...
`
    ...
    const DICT = {
      zh: {
        ...
        footnote: '💡 通配符（文件类工具）：*.pem = 文件名以 .pem 结尾；*secret* = 路径包含 secret；prefix* = 以 prefix 开头；* 单独 = 全部。不带通配符的片段（如 .ssh、appdata）匹配路径任意位置。绝对路径（C:\\、/）按完整前缀匹配。所有修改先进入本页草稿，点击「保存并应用」后立即生效并写入 settings.yaml（跨重启保留）；「放弃修改」恢复为上次保存的状态。workspace 级规则引擎已支持，界面编辑后续补充。引擎关闭时所有规则透明（包括永不允许）。',
      },
      en: {
        title: '🛡️ Permissions',
    ...
    function makeTranslator(ctx) {
      ...
      return {
        t(key, params) {
          ...
          if (face !== undefined) return face.t(key, params)
    ...
    function PermissionsSettingsPage(props) {
      ...
      const editor = React.createElement('div', { className: 'dp-card' },
        React.createElement('div', { className: 'dp-card-head' },
          React.createElement('span', { className: 'dp-card-title' }, t('editorTitle')),
    ...
    function apply(ctx) {
      ...
      slots.inject('settings.section', () => slots.register(
        { name: 'settings.section', id: 'dsh-permissions', order: 30, label: () => translator.t('title') },
        () => React.createElement(PermissionsSettingsPage, { translator }),
      ))
    }
    exports.apply = apply
