...
`
    ...
    const DICT = {
      ...
      en: {
        ...
        badgeToggleOn: '⏸ Pause engine',
        badgeToggleOff: '▶ Enable engine',
    ...
    function PermissionsBadge(props) {
      ...
      React.useEffect(() => {
        ...
        return () => { alive = false; window.removeEventListener('dsh-permissions-changed', onRules); clearInterval(timer) }
      }, [])
      React.useEffect(() => {
      ...
      const count = info.enabled
        ...
        : -1
      const topHits = hits.slice().sort((a, b) => b.count - a.count).slice(0, 5)
      return React.createElement('div', { className: 'dp-badge-wrap' },
        React.createElement('div', {
          className: info.enabled ? 'dp-badge' : 'dp-badge dp-badge-off',
          title: info.enabled ? t('badgeOn', { n: count }) : t('badgeOff'),
          onClick: () => setOpen(!open),
        }, '🛡 ' + (info.enabled ? String(count) : 'OFF')),
        open ? React.createElement('div', { className: 'dp-popover' },
          React.createElement('div', { className: 'dp-popover-row' },
            ...
            React.createElement('span', { style: { marginLeft: 'auto', opacity: .7 } }, info.enabled ? 'ON' : 'OFF')),
          info.enabled ? [
            React.createElement('div', { key: 'counts', className: 'dp-popover-row' },
              t('panelCount', { n: (info.hard || []).length }) + ' hard · '
              + t('panelCount', { n: (info.deny || []).length }) + ' deny · '
              + t('panelCount', { n: (info.ask || []).length }) + ' ask · '
              + t('panelCount', { n: (info.allow || []).length }) + ' allow'),
          ] : null,
          topHits.length > 0 ? topHits.map((h) => React.createElement('div', { key: h.source + h.action + h.raw, className: 'dp-popover-row' },
            React.createElement('span', { className: 'dp-dot dp-dot-' + h.action }),
            React.createElement('span', null, h.raw),
            React.createElement('b', { style: { marginLeft: 'auto' } }, '×' + h.count))) : null,
          React.createElement('div', { className: 'dp-popover-row', style: { borderTop: '1px solid var(--dsw-alias-border-l1)', paddingTop: 6, marginTop: 2 } },
            React.createElement('button', {
              className: info.enabled ? 'dp-btn-ghost dp-btn' : 'dp-btn',
              style: { padding: '3px 10px', fontSize: 11 },
              onClick: () => {
                const next = Object.assign({}, info, { enabled: !info.enabled })
                fetch('/api/dperm/rules', {
                  method: 'POST',
                  headers: { 'content-type': 'application/json' },
                  ...
                  .then((j) => {
                    if (j && j.ok) {
                      setInfo(j.value)
                      try { window.dispatchEvent(new CustomEvent('dsh-permissions-changed')) } catch (e) {}
                    }
                  })
                  .catch(() => {})
              },
            }, info.enabled ? t('badgeToggleOn') : t('badgeToggleOff')),
            React.createElement('span', { style: { marginLeft: 'auto', opacity: .7, fontSize: 10 } }, t('badgePopoverHint')),
          ),
        ) : null)
    }
