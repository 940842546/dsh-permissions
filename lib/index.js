...
export function apply(ctx) {
  // ---------- 自管 JSON 持久化（替代 settings 命名空间，兼容 0.1.7 的 settings 重构） ----------
  // dsh ≥ 0.1.7-alpha.1 移除了 ctx.settings.register（SettingsForms 重构）；
  // 改为自管 JSON 文件，读写路径与旧 settings.yaml 中的规则互不冲突。
  // 迁移：首次运行时若 settings.yaml 有 dsh-permissions 段则一次性导入。
  const dshHome = process.env.DSH_HOME || path.join(homedir(), '.dsh')
  const stateFile = path.join(dshHome, 'dsh-permissions-state.json')
  let current = {
    enabled: true,
    hard: DEFAULT_HARD.slice(),
    deny: DEFAULT_DENY.slice(),
    ask: DEFAULT_ASK.slice(),
  ...
  const loadState = async () => {
    ...
    try {
      ...
      // 首次运行：尝试从旧 settings.yaml 迁移
      try {
        const yamlPath = path.join(dshHome, 'settings.yaml')
        const yaml = await readFile(yamlPath, 'utf8')
        const m = yaml.match(/dsh-permissions:\n((?:[ \t]+.*\n?)*)/)
        if (m) {
          // 极简 YAML 解析（只处理我们自己的扁平结构）
          const section = m[1]
          const parseList = (key) => {
            const km = section.match(new RegExp('[ \\t]+' + key + ':(?:\\n[ \\t]+- ([^\\n]*))+'))
            if (!km) return undefined
            return km[0].split('\n').filter((l) => l.trim().startsWith('- ')).map((l) => l.trim().slice(2).trim())
          }
          const en = section.match(/[ \t]+enabled: (\S+)/)
          const migrated = {}
          if (en) migrated.enabled = en[1] === 'true'
          for (const k of ['hard', 'deny', 'ask', 'allow']) {
            const list = parseList(k)
            if (list !== undefined) migrated[k] = list
          }
          if (Object.keys(migrated).length > 0) {
            current = Object.assign({}, current, migrated)
  ...
  // 缓存与修订号声明在 saveState 之前（loadState 的异步回调可能在声明之后才执行，
  // 但安全起见把声明放最前，避免 TDZ 问题）
  let rulesRev = 0
  ...
  if (webServer !== undefined) {
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        ...
        // 浏览器认证（dsh ≥ 0.1.2-alpha.2）：connection.isAuthenticated 只做
        // cookie 校验（authorizeIndex 会写 303/401 响应且仅适用于 / 路径，
        // 对 API 路由会误拦）。旧版无此方法则放行。
        try {
