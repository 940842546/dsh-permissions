...
3. **Never pass unbounded method references to React's `useSyncExternalStore`** (`controller.subscribe` loses `this` and crashes the render). Corollary: **every hook (useState/useRef/useEffect/…) must sit above any early `return`** — a hook below a conditional return changes the hook count between renders and crashes with "Rendered more hooks than during the previous render" (bit us in v1.0.9).
